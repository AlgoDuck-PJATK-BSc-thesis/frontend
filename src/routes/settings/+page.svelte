...
<section class="flex justify-center gap-14 p-8 h-[calc(100vh-8rem)] overflow-hidden">

	<div class="flex flex-col gap-2 w-64 flex-shrink-0">
  		<h2 class="font-bold text-5xl text-[color:var(--color-primary)] mt-2 mb-6 text-center"
		style="font-family: var(--font-ariw9500);">Settings</h2>
		<div class="w-70 h-[57vh] overflow-y-auto border-2 border-[color:var(--color-accent-1)] rounded-lg p-4 bg-[color:var(--color-tile)]  flex-shrink-0">
			<ul>
				<li class="font-normal text-[color:var(--color-accent-2)] mt-2">Personal Details</li>
				<li><button onclick={() => scrollTo('profile')} class="block w-full text-left text-sm p-2 hover:bg-[color:var(--color-primary)] hover:text-[color:var(--color-tile)] rounded">Profile Information</button></li>
				<li><button onclick={() => scrollTo('language')} class="block w-full text-left text-sm p-2 hover:bg-[color:var(--color-primary)] hover:text-[color:var(--color-tile)] rounded">Display Language</button></li>
				
				<li class="font-normal text-[color:var(--color-accent-2)] mt-8">Security</li>
				<li><button onclick={() => scrollTo('password')} class="block w-full text-sm text-left p-2 hover:bg-[color:var(--color-primary)] hover:text-[color:var(--color-tile)] rounded">Password Settings</button></li>
				<li><button onclick={() => scrollTo('2fa')} class="block w-full text-sm text-left p-2 hover:bg-[color:var(--color-primary)] hover:text-[color:var(--color-tile)] rounded">Two-Factor Authentication</button></li>

				<li class="font-normal text-[color:var(--color-accent-2)] mt-8">Notifications</li>
				<li><button onclick={() => scrollTo('email')} class="block w-full text-left text-sm p-2 hover:bg-[color:var(--color-primary)] hover:text-[color:var(--color-tile)] rounded">Email Notifications</button></li>
				<li><button onclick={() => scrollTo('push')} class="block w-full text-left text-sm p-2 hover:bg-[color:var(--color-primary)] hover:text-[color:var(--color-tile)] rounded">Push Notifications</button></li>
	...
	<div class="flex flex-col gap-5 overflow-y-auto pr-4 flex-[0.5]">
		...
		<div id="language" class="border-2 border-[color:var(--color-accent-1)] bg-[color:var(--color-tile)] p-4 rounded-xl flex flex-col gap-4">
			<h2 class="text-base font-normal text-[color:var(--color-accent-2)]">Display Language</h2>
			...
			<div class="flex gap-4 text-sm text-[color:var(--color-text)]">
				<label class="flex items-center gap-2 cursor-pointer">
					<input
						...
						class="hidden peer"
					/>
					<div class="w-4 h-4 rounded-full border-2 border-[color:var(--color-primary)] bg-[color:var(--color-tile)] flex items-center justify-center peer-checked:after:bg-[color:var(--color-primary)] after:content-[''] after:w-2 after:h-2 after:rounded-full after:bg-[color:var(--color-tile)] transition-colors"></div>
					English
				</label>
		...
		<div id="password" class="border-2 border-[color:var(--color-accent-1)] bg-[color:var(--color-tile)] p-4 rounded-xl flex flex-col gap-6">
			...
			<button
				...
				class="self-end px-[0.8rem] py-[0.3rem] bg-[color:var(--color-primary)] text-[color:var(--color-tile)] text-xs font-body rounded cursor-pointer border-none">Save
			</button>
			{#if showSavedPassword}
				<p class="text-[0.75rem] text-[color:var(--color-accent-2)] mt-[0.3rem] text-right">Changes saved!</p>
			{/if}
		</div>
