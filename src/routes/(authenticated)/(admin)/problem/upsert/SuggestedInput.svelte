<script lang='ts' generics='T extends Record<string, any>'>
    ...
    let selectedCapture: T | undefined = $state();
    let SelectedDisplayComponent: Component<{ options: RecommendationDisplayCompArgs<T> }> = $derived(
        options.SelectedDisplayComp ?? options.DisplayComp
    );
            ...
            <div class="w-full bg-[#2d2d2d] border border-[#007fd4] rounded-sm overflow-hidden">
                <div class="flex items-center">
                    <div class="flex-1 pointer-events-none">
                        <SelectedDisplayComponent options={{
                            onSelect: () => {},
                            content: selectedCapture
                        }}/>
                    </div>
                    <button 
                        onclick={clearSelection}
                        class="flex items-center justify-center w-8 h-8 hover:bg-[#37373d] transition-colors shrink-0"
                        title="Clear selection"
                    >
                        <CrossIconSvg options={{ class: "w-3.5 h-3.5 stroke-[#858585] stroke-2 hover:stroke-[#cccccc]" }}/>
                    </button>
                </div>
            </div>
        </div>
                ...
                <div class="max-h-60 overflow-y-auto">
                    {#each suggestions as suggestion}
                        <DisplayComponent options={{
                            onSelect: (selected: T) => handleSelect(selected),
                            content: suggestion
                        }}/>
                    {:else}
