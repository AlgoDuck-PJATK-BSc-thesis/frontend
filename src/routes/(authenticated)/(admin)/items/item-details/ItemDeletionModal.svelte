<script lang='ts'>
    ...
    let { isVisible = $bindable(), itemId }: { isVisible: boolean, itemId: string } = $props();
    ...
    const deleteProblem = async () => {
        FetchFromApi<DeleteProblemResult>("admin/item", {
            method: "DELETE"
        }, fetch, new URLSearchParams({ itemId: itemId })).then((value: StandardResponseDto<DeleteProblemResult>) => {
            deleteProblemResult = { 
                wasDeleted: value.status === "Success",
                message: value.message || "Item deleted sucesfully. \nClosing this window will redirect back to item list" 
            }
        }).catch((err) => {
...
<main class="bg-black/25 w-screen h-screen fixed inset-0 z-999 flex justify-center items-center">
    <div use:clickOutside={closeModal} class="w-120 relative flex flex-col bg-admin-bg-secondary gap-4 border border-admin-border-primary rounded-xl p-5 pb-1">
        <button onclick={closeModal} class="absolute w-6 h-6 p-0.5 top-2 right-2 hover:bg-admin-bg-hover hover:stroke-admin-text-secondary transition-colors ease-out duration-200 rounded-md">
            <CrossIconSvg options={{ class:'w-full h-full stroke-admin-text-muted stroke-[2]'}}/>
        </button>
        <h3 class="text-lg text-admin-text-primary border-b border-b-admin-border-primary py-1">Delete Item</h3>
        <div class="flex flex-col gap-2">
            <span class="text-sm">Item <i>{itemId}</i> will be deleted.<br/> This action is <strong class="text-admin-danger-text">Irreversable</strong></span>
            ...
            <div class="w-full text-sm flex-row gap-1 flex justify-end py-2">
                <button onclick={closeModal} class="h-10 px-8 transition-colors duration-300 ease-out rounded-md hover:bg-admin-bg-hover">Cancel</button>
            ...
            {#if deleteProblemResult}
                <div class="w-full flex flex-row min-h-10 items-center p-3 rounded-lg {deleteProblemResult.wasDeleted ? "bg-admin-success/20" : "bg-admin-danger-bg/20"}">
                    ...
                    {/if}
                    <div class="grow text-sm whitespace-pre">
