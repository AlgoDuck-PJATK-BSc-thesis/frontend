...
export const simulateFetchAsync = (pageNumber: number): DuckShopPage => {
	...
	const fetched: Array<DuckDto> = outfitCloudfrontUuids.map((i) => {
		return { id: i };
	});
...
export const loadDucks = async (): Promise<DuckDto[]> => {
	const page: DuckShopPage = simulateFetchAsync(0);
	return page.ducksPaged;
};
