<script lang="ts">
	...
	function run() {
		...
		while (l <= r && t >= a[l] && t <= a[r]) {
			...
			const pos = l + Math.floor(((t - a[l]) * (r - l)) / (a[r] - a[l]));
		...
		intervalId = setInterval(() => {
			if (i < steps.length) {
				current = steps[i];
				status = `Probe index ${current} (value ${a[current]})`;
	...
	<div class="grid gap-6 md:grid-cols-2">
		<div class="rounded-3xl p-6">
			...
			<div class="mt-2 text-sm text-[color:var(--color-landingpage-subtitle)]">
				Estimates the position using value distribution (best on uniformly distributed sorted
				arrays).
			</div>
