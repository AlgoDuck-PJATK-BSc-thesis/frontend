...
<div class="grid gap-6 md:grid-cols-2">
	<div class="rounded-3xl border border-white/10 bg-white/5 p-6">
		...
		<div class="mt-2 text-sm text-[color:var(--color-landingpage-subtitle)]">
			Builds an MST by sorting edges and adding the next cheapest edge that doesn’t create a cycle.
		</div>
			...
			<div class="mt-4 text-center text-sm text-[color:var(--color-landingpage-subtitle)]">
				{#if mst.length}
					<div class="font-semibold text-white">
						MST: {mst.map((e) => `${e.a}-${e.b}(${e.w})`).join(', ')}
					</div>
				{:else}
					Run to build the spanning tree.
				{/if}
			</div>
