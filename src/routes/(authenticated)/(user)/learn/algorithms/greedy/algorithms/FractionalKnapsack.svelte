<script lang="ts">
	...
	function run() {
		...
		steps.push({ considering: null, taken: [], total: 0, remaining: rem });

		for (const item of sorted) {
			steps.push({ considering: item.name, taken: [...localTaken], total, remaining: rem });
		...
		steps.push({ considering: null, taken: [...localTaken], total, remaining: rem });
		...
		<div class="mt-6 rounded-2xl border border-white/10 bg-slate-900/60 p-5">
			...
			<div class="mt-6">
				...
				<div class="mt-3 grid gap-3 sm:grid-cols-3">
					{#each [...items].sort((a, b) => b.value / b.weight - a.value / a.weight) as item}
						<div
							class={[
			...
			<div class="mt-6 rounded-2xl border border-white/10 bg-white/5 p-4">
				...
				<div class="mt-3 h-2 overflow-hidden rounded-full bg-slate-700">
					class="h-full bg-emerald-500" style={`width:${pctUsed()}%`}
				</div>
