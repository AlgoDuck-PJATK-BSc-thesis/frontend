<script lang="ts">
	...
	function layout(
		...
		nodes = [...nodes, { x, y, value: node.value, key }];

		if (parent) {
			edges = [...edges, { fromX: parent.x, fromY: parent.y + 18, toX: x, toY: y - 18 }];
		}

		const childY = y + 80;
		const delta = spread / (level + 1);
		layout(node.left, x - delta, childY, level + 1, spread, { x, y });
		layout(node.right, x + delta, childY, level + 1, spread, { x, y });
	}

	$effect(() => {
		nodes = [];
		edges = [];
		layout(tree, 460, 60, 0, 280);
	});
