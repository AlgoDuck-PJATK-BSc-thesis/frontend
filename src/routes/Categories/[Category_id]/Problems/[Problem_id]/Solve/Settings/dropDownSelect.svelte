...
<main class="w-full h-full bg-ide-card rounded-md relative">
	<button
		...
		class="w-full h-full flex justify-between px-4 z-100 py-2 items-center absolute top-0 hover:cursor-pointer"
	>
	...
	<div
		...
		class="w-full h-0 bg-ide-card rounded-md absolute {isDropDownMenuShown
			? 'p-1'
			: ''} grid grid-cols-1 overflow-x-hidden overflow-y-scroll z-10 gap-y-1"
	>
		{#each options.options as selectionOption}
			...
			>
				<span class="w-full h-full px-4 flex justify-start items-center text-text-primary"
