...
{#snippet VerticalTabDiv()}
  <div class="h-full w-10 flex-shrink-0 bg-ide-dcard border-ide-bg rounded-xl gap-1 flex flex-col py-1.5 items-center justify-start">
    {#each options.components as _, i}
      <button class="w-[80%] aspect-square p-[5%] flex justify-center items-center  hover:cursor-pointer {i === feturedElementIndex ? 'bg-gray-800' : 'hover:bg-gray-400/10'} border-black rounded-md" onclick="{()=>{feturedElementIndex = i}}">
        {#await svgComponents}
...
{#snippet HorizontalTabDiv()}
  <div class="w-full h-10 flex-shrink-0 bg-ide-dcard border-ide-bg rounded-xl gap-2 px-1 flex items-center justify-start overflow-hidden">
    {#each options.components as component, i}
      <button class="h-[80%] hover:cursor-pointer px-5 gap-3 flex flex-row justify-center items-center {i === feturedElementIndex ? 'bg-gray-800' : 'hover:bg-gray-400/10'} border-black rounded-lg" onclick="{()=>{feturedElementIndex = i}}">
