...
<main class="w-full h-full flex flex-col bg-ide-card rounded-lg overflow-hidden border border-ide-accent/10">
    <div class="w-full h-10 bg-ide-dcard flex items-center justify-between px-4 border-b border-ide-accent/10">
        ...
        <span class="text-sm text-ide-text-secondary/60 select-none">
            {options.terminalContents ? 'Running...' : 'Waiting for output'}
        </span>
    </div>
    ...
    >
        {#if options.terminalContents}
            <pre class="text-ide-text-primary whitespace-pre-line break-words">{options.terminalContents}</pre>
        {:else}
