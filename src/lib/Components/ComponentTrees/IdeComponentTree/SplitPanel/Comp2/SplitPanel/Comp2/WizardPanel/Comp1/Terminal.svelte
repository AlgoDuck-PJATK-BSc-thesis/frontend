...
<main class="w-full h-full border border-ide-accent/10 flex flex-col bg-ide-card rounded-lg overflow-hidden">
    ...
    >
        {#if options.stdOut || options.stdErr}
            {#if isError}
                <div class="w-full h-full bg-red-300/20 overflow-y-auto flex flex-col justify-start items-center rounded-2xl p-10 gap-3">
            ...
            {/if}
        {:else}
            <div class="flex items-center justify-center overflow-y-auto h-full text-ide-text-secondary/40">
