  ...
  let Component2Clamp: Component<{ options: T2 }> | undefined = $derived(
	...
	const handleMouseMove = (e: MouseEvent) => {
		...
		if (lastMouseP !== null) {
			...
			if (comp2Proportion - delta > comp2ClampValue && options.initialComp1Proportions + delta > comp1ClampValue){
...
{#if options && options.comp1 && options.comp2}
	<div bind:this={mainContainer} class="h-full flex {options.axis === 0 ? '' : 'flex-col'}">
		{#if Component1 && comp1Options}
			<div bind:this={options.comp1.contentWrapper} class="{options.axis === 0 ? 'h-full' : 'w-full'} overflow-hidden" style="flex: {options.initialComp1Proportions}">
		...
		<button
			...
			class="{options.axis === 0 ? 'h-full w-2.5' : 'w-full h-2.5'} bg-transparent shrink-0 grow-0 flex justify-center items-center"
		...
		{#if Component2 && comp2Options  && options.comp1}
	  	<div bind:this={options.comp2.contentWrapper} class="{options.axis === 0 ? 'h-full' : 'w-full'} overflow-hidden" style="flex: {comp2Proportion}">
